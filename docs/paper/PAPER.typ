...
#let doc-subtitle = none
#let doc-author = none
#let doc-version = none
#let doc-publisher = none
...
#let doc-make-title() = {
  ...
  block(spacing: 2em)[
    ...
    #if doc-authors != none [
      #text(size: 1.1em, str(doc-authors.join(", ")))
    ]
    #if doc-email != none [
      #v(0.2em)
      #text(size: 0.95em, fill: luma(100), doc-email)
    ]
    #v(0.3em)
    #if doc-date != none [
...
#doc-make-title()
  ...
  #block(
    width: 88%,
    inset: (left: 2.5em, right: 2.5em),
  )[
    #set par(justify: true)
    #text(size: 0.95em)[
      #align(center)[*Abstract*]
      #v(0.3em)
      #eval(doc-abstract, mode: "markup")
    ]
  ]
  #v(0.5em)
]
...
\(a ~130:1 ratio) by transmitting _edges and contrast_ rather than absolute \
...
_hallucinate_ the sensory experience back into existence. The brain stores \
...
+ *Paradigm framing.* We articulate the "Semantic-Relational Field / \
generative-recall" image-compression paradigm and map it onto a concrete \
encoder–format–decoder architecture.
+ *A versioned, forward-compatible file format.* `brainimg` v0.1 is a small \
...
+ *A four-stage encoder* combining a vision-language model \(Qwen2-VL on \
...
schema change.
+ *Empirical findings on distilled schedules and scale tuning.* On the \
SIPI mandril test image, Hyper-SD's 8-step distilled LoRA _beats_ the \
...
+ *A gamma fallback for the brightness-clamp edge case.* When the uniform \
gain clamp #mi("[0.5, 2.0]") cannot reach an extreme target, a per-channel \
gamma curve \(same exponent, clamped to #mi("[0.3, 3.0]")) closes the residual \
...
The three dominant compression paradigms are _spatial/predictive_ \(PNG, \
run-length + DEFLATE over filtered scanlines), _spectral/transform_ \
\(JPEG's discrete cosine transform, WebP's VP8 intra, AVIF's AV1 intra), and \
_fractal/wavelet_ \(JPEG 2000). All are pixel-bound: the file encodes a fixed \
grid at a fixed resolution, and "loss" is a transform artefact. brainimg \
departs from all of these by storing _no pixel data at all_.
...
continuous vector but a _human-readable caption plus a few low-resolution \
maps and a seed_, and the decoder is a general-purpose text-to-image \
...
conditioning and a stored seed is, to our knowledge, novel as a _file-format_ \
...
literature. We use these only as _motivation_; brainimg is an engineering \
artefact, not a brain model.

= 3. Method

brainimg has three parts: an _encoder_ that extracts a blueprint from a \
source image, a _file_ that stores it, and a _decoder_ that regenerates an \
...
The format module \(`src/brainimg/format.py`) is deliberately free of any ML \
...
The encoder \(`src/brainimg/extract.py`) runs four extractors in sequence, \
...
the instruction _"In one short sentence, describe this image for \
reconstruction: main objects, layout, lighting, and colors."_ On Apple \
...
post-processing. _Brightness_ is the mean Rec.609 luminance on a 0–255 scale:

#mitex(` B = \mathrm{mean}\!\left(0.299\,R + 0.587\,G + 0.114\,B\right). `)

_Saturation_ is the mean normalised channel spread, scaled to 0–255 \(0 = \
grey, 255 = fully saturated):

#mitex(` S = 255 \cdot \mathrm{mean}\!\left(\frac{\max_c - \min_c}{\max(\max_c, 1)}\right). `)

A short _colour-style prefix_ is also derived \(e.g. _"dark, low-key lighting, \
red dominant tones"_) and stored in `extra`; the decoder prepends it to the \
caption only when the combined length fits the CLIP 77-token limit, so the \
caption itself is never truncated.

== 3.3 Decoder

The decoder \(`src/brainimg/generate.py`) regenerates an image from the blueprint \
...
#table(columns: 5, align: (auto, auto, auto, auto, auto),
  table.header([`--model`], [Base], [Conditioning], [Steps \(default)], [License]),
...
backends are supported by `src/decoder.py` but not enumerated as separate paper \
...
+ _Saturation first_, by scaling the HSV-S channel by \
`target_saturation / current_saturation`, iterated 1–2× to converge \(HSV-S \
is not linear in the metric above). Ratios are clamped to #mi("[0.5, 2.0]") to \
avoid clipping artefacts. This perturbs brightness, which the next step \
corrects.
+ _Brightness last_, by a uniform RGB gain `target_brightness / current_brightness`. A uniform gain preserves colour balance and leaves \
the saturation metric \(a ratio of channels) invariant, so correcting \
brightness last does not undo the saturation work. The same #mi("[0.5, 2.0]") \
clamp applies.
...
#mi("\\text{arr}^\\gamma") \(same exponent on every channel, clamped to \
#mi("[0.3, 3.0]")) to close the residual gap. Gamma preserves colour balance \
...
cause. The gamma exponent is #mi("\\gamma = \\log(t/255) / \\log(c/255)") where #mi("c") \
and #mi("t") are the current and target brightnesses normalised to #mi("[0, 1]").
...
`alibaba-pai/Z-Image-Turbo-Fun-Controlnet-Union-2.1` _Union_ ControlNet \
...
conditioning _type_ \(depth/canny/pose/mlsd/hed) into its training; we feed \
...
or 30.0 \(canny), 30 inference steps, 1024 max side, `max_sequence_length = 512` \(T5-XXL). No per-call `controlnet_conditioning_scale`—the \
...
The current dev target is an *AMD x86\_64 CPU-only box with 188 GB RAM* \
...
ControlNets \(weights _and_ activations) via `optimum-quanto` to avoid fp16 \
...
#table(columns: 5, align: (auto, auto, auto, auto, auto),
  table.header([`--device` / `--model`], [Precision], [RAM], [Speed], [Fidelity]),
...
The encoder/decoder device selection is centralised in `src/brainimg/device.py`, \
...
- *Hardware:* AMD x86\_64 CPU-only, 188 GB RAM \(the current dev target), \
...
#link("https://en.wikipedia.org/wiki/Lenna")[https://en.wikipedia.org/wiki/Lenna]).
...
#table(columns: 4, align: (auto, auto, auto, auto),
  table.header([Source file], [Source size], [`.brainimg` size], [Ratio]),
...
the compression ratio therefore _grows with source size_: the large \
...
#figure(
  image("../grids/peppers_grid.jpg", width: 100%),
  caption: [
...
#figure(
  image("../grids/cameraman_grid.jpg", width: 100%),
  caption: [
...
#figure(
  image("../grids/airplane_grid.jpg", width: 100%),
  caption: [
...
side-by-sides are produced by `src/decoder.py` and `scripts/make_comparison.py` \
...
#mi("[0.5, 2.0]") cannot reach an extreme target, a per-channel gamma curve \
...
#table(columns: 6, align: (auto, right, right, right, right, right),
  table.header([Backend], [Steps], [Time \(s)], [MSE ↓], [PSNR \(dB) ↑], [MAE ↓]),
...
\(9.90 dB) does _not_ beat the 30-step path \(9.92 dB) on this image — \
...
+ *SDXL is the best result across all backends* \(13.01 dB, 989 s) — the \
...
generalise.
+ *ControlNet scale tuning on SD 1.5.* The tuned defaults \(depth 0.8, \
...
+ *Qwen-Image \(Apache 2.0) is competitive with SDXL turbo* \(10.39 dB \
...
+ *Z-Image is the weakest depth-only backend* \(9.67 dB, 308 s) — \
...
1024-native backend.
+ *Pixel-level PSNR does not track perceptual quality for two backends.* \
...
+ *The two newest ControlNet backends sit in the mid-PSNR range.* \
...
*Determinism.* Stored seed + fixed decoder #mi("\\Rightarrow") bit-identical re-decode. This \
addresses the usual "hallucination is non-deterministic" objection to \
generative codecs: within a fixed decoder version, a file _is_ a stable image.
...
is _shared_ across all files, so the per-file cost remains a few KB.
...
hue _distribution_ than the source \(§4.6). HunyuanDiT and FLUX.2-klein \
exhibit a more severe form of this — a palette _collapse/shift_ \
...
these via `src/encoder.py`/`src/decoder.py` and the `samples/` directory.

= 6. Conclusion

brainimg is a small, reproducible prototype of a different way to compress \
images: store the _meaning and structure_ of a scene and let a diffusion model \
...
8-step distilled schedules _beat_ their 30-step counterparts on SD 1.5 \
...
upgrade \(§4.6). The MAP\_SIZE 128→256 bump, ControlNet scale tuning, \
...
python src/encoder.py samples/real.jpg -o outputs/out.brainimg --seed 42
...
Known gotchas \(from `AGENTS.md`): the MPS fp16 NaN bug is _not_ a bug to \
...
documented at #link("https://en.wikipedia.org/wiki/Lenna")[https://en.wikipedia.org/wiki/Lenna]; the numbers below are \
...
FLUX distilled schedule does _not_ beat the 30-step path. The \
...
== A.3 MAP\_SIZE regression \(Lenna)

The 128→256 MAP\_SIZE bump regressed on every backend on Lenna at 512×512: \
...
_High-Resolution Image Synthesis with Latent Diffusion Models_ \(Stable \
Diffusion, arXiv 2112.10752). CVPR 2022.
- Radford, A., Kim, J. W., Hallacy, C., et al. \(2021). _Learning Transferable \
Visual Models from Natural Language Supervision_ \(CLIP, arXiv 2103.00020). \
ICML 2021. CLIP text embeddings condition SD 1.5 / SDXL; CLIP-score is a \
planned semantic-fidelity metric \(§5.3).
- Zhang, L., Rao, A., Agrawala, M. \(2023). _Adding Conditional Control to \
Text-to-Image Diffusion Models_ \(ControlNet, arXiv 2302.05543). ICCV 2023.
- Yang, L., Kang, B., Huang, Z., Zhao, Z., Xu, X., Feng, J., Zhao, H. \(2024). \
_Depth Anything V2_ \(Depth-Anything-V2-Base, arXiv 2406.09414). NeurIPS 2024.
- Jain, J., Li, J., Chiu, M.-T., et al. \(2023). _OneFormer: One Transformer to \
Rule Universal Image Segmentation_ \(arXiv 2211.06220, CVPR 2023; \
`shi-labs/oneformer_ade20k_swin_tiny`).
- Wang, P. et al. \(2024). _Qwen2-VL: Enhancing Vision-Language Model's \
Perception of the World at Any Resolution_ \(arXiv 2409.12191). \
...
Penna, J., Rombach, R. \(2023). _SDXL: Improving Latent Diffusion Models for \
High-Resolution Image Synthesis_ \(arXiv 2307.01952). Model: \
`stabilityai/stable-diffusion-xl-base-1.0`.
- Esser, P., Kulal, S., et al. \(2024). _Scaling Rectified Flow Transformers \
for High-Resolution Image Synthesis_ \(Stable Diffusion 3, arXiv 2403.03206). \
...
- Black Forest Labs. _FLUX.1-dev_ \(12 B MMDiT + T5-XXL, rectified flow; \
...
- Shakker Labs & InstantX \(2024). _FLUX.1-dev-ControlNet-Union-Pro_ \(Union \
...
- Ren, Y., Xia, X., Lu, Y., et al. \(2024). _Hyper-SD: Trajectory Segmented \
Consistency Model for Efficient Image Synthesis_ \(arXiv 2404.13686). \
...
- Song, J., Meng, C., Ermon, S. \(2021). _Denoising Diffusion Implicit \
Models_ \(DDIM, arXiv 2010.02502). ICLR 2021. Used by the Hyper-SD turbo \
schedulers \(`DDIMScheduler(timestep_spacing="trailing")`\).
- Wu, C., Li, J., Zhou, J., et al. \(2025). _Qwen-Image Technical Report_ \
\(arXiv 2508.02324). `Qwen/Qwen-Image` \(Apache 2.0 DiT) with \
`InstantX/Qwen-Image-ControlNet-Union` \(Union ControlNet, Apache 2.0).
- Li, Z., Zhang, J., Lin, Q., et al. \(2024). _Hunyuan-DiT: A Powerful \
  Multi-Resolution Diffusion Transformer with Fine-Grained Chinese \
  Understanding_ \(arXiv 2405.08748). `Tencent-Hunyuan/HunyuanDiT-v1.2-Diffusers-Distilled` and `HunyuanDiT-v1.2-Diffusers` \(non-distilled) with \
  separate `HunyuanDiT-v1.2-ControlNet-Diffusers-{Depth,Canny}` \
  \(tencent-hunyuan-community license).
- Xie, E., Chen, J., Chen, J., et al. \(2024). _SANA: Efficient \
High-Resolution Image Synthesis with Linear Diffusion Transformers_ \
...
- Black Forest Labs. _FLUX.2-klein-4B_ \(Apache 2.0, ungated, 4 B \
...
- Wallace, G. K. \(1992). _The JPEG Still Picture Compression Standard._ \
...
- Olshausen, B. A., Field, D. J. \(1996). _Emergence of simple-cell receptive \
field properties by learning a sparse code for natural images_ \(sparse \
coding / brain motivation).
- Rao, R., Ballard, D. \(1999). _Predictive coding of natural images_ \
\(predictive coding / brain motivation).
- Hubel, D., Wiesel, T. \(1959/1962). _Receptive fields of single neurons in \
the cat's striate cortex_ \(V1 feature extraction motivation).
